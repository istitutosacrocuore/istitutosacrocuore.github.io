...
class AnswersViewController: UITableViewController, PlaygroundLiveViewMessageHandler, InputCellDelegate {
    ...
    func display(_ image: UIImage) {
        append(item: TranscriptItem(image: image), animated: true)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if item.isUserEntered {
            ...
            imageCell.displayImage = image
            
            cell = imageCell
        } else {
            let textCell = tableView.dequeueReusableCell(withIdentifier: MessageCell.reuseIdentifier, for: indexPath) as! MessageCell
            textCell.messageText = item.text
            textCell.layoutMargins.right = InputCell.submitButtonWidthPlusSpacing + InputCell.defaultLayoutMargins.right
